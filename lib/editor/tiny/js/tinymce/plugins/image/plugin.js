...
(function () {
    ...
    const changeFileInput = (helpers, info, state, api) => {
      const data = api.getData();
      api.block('Uploading images');
      
      const files = data.fileinput;
      ...
      const uploadPromises = files.map(file => {
        ...
        return blobToDataUri(file).then(dataUrl => {
          const blobInfo = helpers.createBlobCache(file, blobUri, dataUrl);
          if (info.automaticUploads) {
            return helpers.uploadImage(blobInfo).then(result => {
            ...
            return Promise.resolve(blobInfo.blobUri());
          }
        });
      });
      ...
      Promise.all(uploadPromises).then(urls => {
        ...
        if (validUrls.length > 0) {
          // For multiple images, we'll insert them one after another
          validUrls.forEach((url, index) => {
            const imageData = {
              ...
              alt: '', // Disable alt text for multiple images
              isDecorative: true // Set as decorative by default for multiple images
            };
            
            if (index === 0) {
              api.setData(imageData);
              api.showTab('general');
              changeSrc(helpers, info, state, api);
            } else {
              // Insert additional images directly
              editor.execCommand('mceInsertContent', false, `<img src="${url}" alt="" role="presentation"/>`);
            }
          });
          api.focus('src');
        }
