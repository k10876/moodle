...
export class ImageInsert {
    ...
    init = async function() {
        ...
        // Add cleanup code for modal closing
        this.currentModal.on('close', () => {
     ...
     * @param {string|null} message - Custom message to display (optional).
     * @returns {void}
     */
    updateLoaderIcon = (root, langStrings, progress = null, message = null) => {
        const loaderIcon = root.querySelector(Selectors.IMAGE.elements.loaderIconContainer + ' div');
        if (message) {
            loaderIcon.innerHTML = message;
        } else {
        ...
        this.root.addEventListener('click', async(e) => {
            ...
            if (imageBrowserAction && this.canShowFilePicker) {
                ...
                if (params.files && params.files.length > 0) {
                    // Handle multiple files from file picker
                    const fileUrls = params.files.map(file => ({ url: file.url }));
                    fileUrls.forEach(fileData => {
                        this.filePickerCallback(fileData);
                    });
                } else if (params.url) {
    ...
    handleFileUpload(files) {
        ...
        this.startImageLoading();
        this.updateLoaderIcon(this.root, this.langStrings, 0);
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            promises.push(
                this.processImageFile(file).then(imageData => {
                    processedFiles++;
                    const progress = (processedFiles / totalFiles) * 100;
                    this.updateLoaderIcon(
                        this.root, 
                        this.langStrings, 
                        progress,
                        `${this.langStrings.multiple_images_processing.replace('{$a}', totalFiles)}`
                    );
                    return imageData;
                })
            );
        ...
        Promise.all(promises).then((images) => {
            ...
            const successMsg = this.langStrings.multiple_images_success.replace('{$a}', validImages.length);
            this.showNotification(successMsg, 'success');
            
            this.modal.destroy();
        }).catch((error) => {
            window.console.error('Error processing images:', error);
            this.showNotification(error.message || 'Error processing images', 'error');
            this.stopImageLoading();
        });
    }
    ...
    setImage(url, properties = {}) {
        const figure = this.editor.dom.create('figure', {class: 'image'});
        const img = this.editor.dom.create('img', {
            src: url,
            alt: properties.alt || '',
            width: properties.width || '',
            height: properties.height || '',
            role: properties.presentation ? 'presentation' : null,
            style: properties.customStyle || ''
        });
